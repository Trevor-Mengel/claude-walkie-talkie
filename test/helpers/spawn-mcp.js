// Launching `bin/walkie-talkie-mcp.js` as a child process.
//
// Two v0.2 inputs are gone and must not be reintroduced here:
//
//   - `WALKIE_TOOL` / `WALKIE_ALIAS`. They were identity: the child declared "I am claude-code
...
//
// What the child DOES read is `WALKIE_CAPABILITY`: a supervisor-injected credential, either a
...
export const MCP_BIN = join(__dirname, '..', '..', 'bin', 'walkie-talkie-mcp.js');

/**
 * Serialise a capability for `WALKIE_CAPABILITY`. A string is passed through as a bare token; an
 ...
 * @param {string} [opts.cwd] defaults to the project root named by `WALKIE_PROJECT_ROOT`
 * @param {string|object} [opts.capability] injected as `WALKIE_CAPABILITY`
 */
