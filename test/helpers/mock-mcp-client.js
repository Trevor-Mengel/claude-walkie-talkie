// A connected MCP client driving a real `collabcast-mcp` child process.
//
// The helper surface tracks the current tool set exactly: the eight preserved v0.2 names plus
// `collabcast_enroll` and `collabcast_ack`. The v0.2 helpers for deleted operations are gone —
 ...
 * @param {string} [opts.cwd] defaults to `env.COLLABCAST_PROJECT_ROOT`
 * @param {string|object} [opts.capability] injected as `COLLABCAST_CAPABILITY`; omit for an
 *   unenrolled session
...
export async function spawnMockClient({ env, cwd, capability, name = 'mock' } = {}) {
  ...
  return {
    ...
    // --- the ten tools -------------------------------------------------------------------
    enroll: (args = {}) => call('collabcast_enroll', args),
    inbox: (args = {}) => call('collabcast_inbox', args),
    ack: (id, args = {}) => call('collabcast_ack', { id, ...args }),
    read: (args = {}) => call('collabcast_read', args),
    talk: (body, args = {}) => call('collabcast_talk', { body, ...args }),
    reply: (replyTo, body) => call('collabcast_reply', { reply_to: replyTo, body }),
    edit: (id, body) => call('collabcast_edit', { id, body }),
    archive: (id, reason) => call('collabcast_archive', { id, ...(reason ? { reason } : {}) }),
    sessions: () => call('collabcast_sessions', {}),
    rename: (alias) => call('collabcast_rename', { alias }),
