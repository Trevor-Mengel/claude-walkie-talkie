...
// while the child auto-joined a session from `COLLABCAST_TOOL`. Both halves are gone: the child is
// handed a capability, and the inventory grew the two tools that carry the authority model —
// `collabcast_enroll` (how a session acquires a capability) and `collabcast_ack` (what acknowledgement
...
// edit here and a silently reintroduced `collabcast_join` / `collabcast_permit` fails.
...
describe('MCP scaffold', () => {
  ...
  test('the removed v0.2 tools are absent, not merely undocumented', async () => {
    const stack = await createStack({
      namespace: 'collabcast-scaffold2',
      roles: ['root', { name: 'hub', role: 'goal_hub' }]
    });
    const client = await spawnMockClient({
      env: stack.childEnv(),
    ...
    try {
      ...
      for (const gone of ['collabcast_join', 'collabcast_invite', 'collabcast_permit', 'collabcast_remove']) {
        expect(names).not.toContain(gone);
      }
      const attempt = await client.callRaw('collabcast_permit', { mode: 'always' });
  ...
  test('a session with no injected capability comes up and says what to do', async () => {
    const stack = await createStack({ namespace: 'collabcast-scaffold3', roles: ['root'] });
    const client = await spawnMockClient({ env: stack.childEnv() });
    try {
      // Listing tools needs no authority: the model has to be able to see collabcast_enroll.
      expect((await client.listTools()).map((t) => t.name)).toContain('collabcast_enroll');

      const blocked = await client.callRaw('collabcast_read', {});
      expect(blocked.isError).toBe(true);
      expect(blocked.payload.code).toBe('unauthenticated');
      expect(blocked.payload.message).toMatch(/collabcast_enroll/);
    } finally {
      await client.close();
    }
  }, 20000);
  ...
  test('a rejected injected capability is named on stderr and the session still comes up', async () => {
    const stack = await createStack({ namespace: 'collabcast-scaffold4', roles: ['root'] });
    ...
    expect(stderr).toMatch(/\[collabcast-mcp] injected capability rejected \(unauthenticated\)/);
    // A rejected injection is reported, not fatal: the session must stay up so the model can
    // still reach collabcast_enroll.
  ...
  test('a usable injected capability produces no startup diagnostic at all', async () => {
    const stack = await createStack({
      namespace: 'collabcast-scaffold5',
      roles: ['root', { name: 'hub', role: 'goal_hub' }]
    });
