// `collabcast://` resources, driven through a real MCP child against a real service.
//
...
// arrives. The fourth changed shape — `collabcast://sessions/active` used to prove "the auto-joined
...
// `collabcast://channel/inbox` MOVES NO CURSOR. A resource read is a passive fetch an MCP client may
...
describe('collabcast:// resources', () => {
  test('lists three resources', async () => {
    const { client } = await harness('collabcast-res1');
    try {
  ...
  test('reading channel/recent returns latest messages', async () => {
    const { stack, client } = await harness('collabcast-res2');
    try {
      await postAsRoot(stack, 'hi');
      await postAsRoot(stack, 'and again');

      const payload = await client.readResource('collabcast://channel/recent');
  ...
  test('reading sessions/active returns the principal roster', async () => {
    const { stack, client } = await harness('collabcast-res3');
    try {
      const payload = await client.readResource('collabcast://sessions/active');
  ...
  test('reading the inbox resource moves no cursor, however many times it is read', async () => {
    const { stack, client } = await harness('collabcast-res4');
    ...
    try {
      ...
      for (let i = 0; i < 3; i += 1) {
        const payload = await client.readResource('collabcast://channel/inbox');
      ...
      await client.readResource('collabcast://channel/recent');
      await client.readResource('collabcast://sessions/active');
      expect(stack.cursors(hubId)).toEqual({ read: '', ack: '' });

      // Only the explicit tool does.
      await client.ack(two);
      expect(stack.cursors(hubId)).toEqual({ read: two, ack: two });
      expect((await client.readResource('collabcast://channel/inbox')).messages).toEqual([]);
    } finally {
      await client.close();
    }
  }, 20000);
  ...
  test('subscribe emits resources/updated when another principal posts', async () => {
    const { stack, client } = await harness('collabcast-res5');
    try {
      /** @type {string[]} */
      const updated = [];
      ...
      await client.subscribe('collabcast://channel/inbox');
      ...
      expect(updated).toContain('collabcast://channel/inbox');
  ...
  test('a subscriber is not woken by its own post', async () => {
    const { stack, client } = await harness('collabcast-res6');
    try {
      /** @type {string[]} */
      const updated = [];
      client.raw.setNotificationHandler(ResourceUpdatedNotificationSchema, (n) => {
  ...
  test('an unknown resource is not_found rather than a crash', async () => {
    const { client } = await harness('collabcast-res7');
    try {
      await expect(client.readResource('collabcast://channel/secrets')).rejects.toThrow(
        /no collabcast resource/
      );
    } finally {
      await client.close();
    }
